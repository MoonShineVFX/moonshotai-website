  ...
  return (
    <div className='fixed z-50 bottom-0 left-0 w-full'>
      ...
      <motion.div 
        ...
        exit={{ opacity: 0, y: -20 }}
        className='bg-gray-900 border border-white/30   rounded-md p-4 box-border text-white fixed top-[20%] left-1/2 -translate-x-1/2 w-4/5 md:w-2/5 overflow-y-auto max-h-[85vh]'
      >
          ...
          <form onSubmit={handleSubmit(onSubmit)} className='my-4'>
                ...
                render={({ field }) => (
                  <div className='flex flex-col'>
                   <div className='text-sm text-white/60 my-1'> Email (用於發票寄送，請務必填寫。)</div>
                    <input
                      {...field}
                      className='bg-gray-700 rounded-md py-2 px-2 text-sm'
            ...
            <div className='mt-6 flex flex-col gap-3 justify-start text-md'>
              ...
              <Button type="submit" className="rounded-full bg-t_lime-600 text-white font-bold capitalize text-base ">
                確認並繼續 
              </Button>
