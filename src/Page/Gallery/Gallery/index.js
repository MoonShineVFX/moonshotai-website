...
import {
  ...
  useDelACollectionImageMutation,
  useLikeImageMutation,
  useDelLikedImageMutation,
...
function Index() {
  ...
  const [isShowimageModal, setIsShowImageModal] =
  ...
  //ADD Collection
  const collectionAImageMutation = useCollectionImageMutation(linLoginData, [
    "galleries",
    linLoginData,
    startDate,
    currModels,
  ...
  //DEL Collection
  const unCollectionAImageMutation = useDelACollectionImageMutation(
    linLoginData,
    ["galleries", linLoginData, startDate, currModels]
  );
  //ADD LIKE
  const likeAImageMutation = useLikeImageMutation(linLoginData, [
    "galleries",
    linLoginData,
    startDate,
    currModels,
  ...
  //DEL LIKE
  const unLikeAImageMutation = useDelLikedImageMutation(linLoginData, [
    "galleries",
    linLoginData,
    startDate,
    currModels,
  ...
  return (
    <div className="w-full ">
      <AnimatePresence>
        {isLoginForFuns && (
          <CallToLoginModal closeModal={() => setIsLoginForFuns(false)} />
        )}
      </AnimatePresence>
      <div className="">
        {!imageData ? (
          ...
          <div>
            <div className="flex items-center mt-6 mb-4 gap-2  justify-between w-full   ">
                ...
                <div className="flex  space-x-3">
                  {filterModelsDate.map((item, index) => {
                    return (
                      <button
                        key={item.title}
                        className={`px-3 py-2 text-xs md:text-sm font-semibold  rounded-full hover:brightness-110 ${
                          currModels === item.value
              ...
              <ImgFilter
                filterItems={filterDateItem}
                defaultIndex={3}
                onHandleSelect={onHandleSelectDate}
                icon="MdAccessTime"
              />
            </div>
            {imageData.length === 0 && (
              <div className="text-white/60 text-sm my-6 text-center">
                這個選擇下目前沒有圖片。
              </div>
            )}
                          ...
                          <div className="ml-auto flex gap-1 justify-end items-center text-white   transition duration-700 ">
                            <div className="flex items-center space-x-2 text-sm">
                              <div
                                ...
                                onClick={() => handleLike(image, is_like)}
                              >
                                {is_like ? (
                                  <FaHeart color="red" />
                                ) : (
