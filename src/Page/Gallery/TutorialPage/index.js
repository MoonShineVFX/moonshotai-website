...
function Index() {
  ...
  return (
    <div className=' fixed top-0 left-0 w-full h-screen bg-black/90 z-50'>
      <div className='bg-white absolute  rounded-full right-5 top-10'><MdClose size={28} /></div>
      <Swiper
          spaceBetween={30}
          slidesPerView={1}
          loop={true}
        ...
        <SwiperSlide>
          ...
          <div className=' relative text-white py-10 px-2'>
            ...
            <div className='text-sm my-2 px-4'>
              在 MoonShot 中繪製的圖片，會依序顯示到 Renders 頁面中，如果要做進一步的分享或編輯圖片資料，必須將圖片留存到 Storage 頁面。
            </div>
            <div className='text-xs mt-10 text-center text-white/80'> 按下加入留存按鈕 將圖片儲存到 Storage </div>
            <div className='w-1/2 mx-auto my-2'>
              ...
              >
                ...
                <div className=' absolute right-1 bottom-2 z-10 text-yellow-500 '><MdAdsClick size={35} /></div>
              </motion.div>
            </div>

            <div className='text-xs mt-10 text-center text-white/80'> 按下移除留存按鈕，取消儲存此圖片。 </div>
            <div className='w-1/2 mx-auto my-2'>
              <motion.div key={'render-'} 
                variants={imageVariants} initial="hidden" animate="visible"
                className=' overflow-hidden relative'
